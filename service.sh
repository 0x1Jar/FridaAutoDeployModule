...
wait_for_network() {
  ...
  log_msg "Checking network connectivity..."
  
  while [ $waited -lt $max_wait ]; do
    if ping -c 1 8.8.8.8 > /dev/null 2>&1; then
      log_msg "✓ Network is available"
      return 0
    fi
    
    log_msg "Network not ready, waiting... ($waited/$max_wait seconds)"
    sleep 2
    waited=$((waited + 2))
  done
  
  log_msg "WARNING: Network still not available after ${max_wait}s, proceeding anyway..."
  return 1
}
...
# Function to download and update Frida with retry logic
update_frida() {
  local version="$1"
  local arch="$2"
  local binary_name="$3"
  ...
  log_msg "Attempting to download Frida $version for $arch (max $MAX_RETRIES retries)..."
  ...
  while [ $retry_count -lt $MAX_RETRIES ]; do
    log_msg "Download attempt $((retry_count + 1))/$MAX_RETRIES..."
    
    # Try to download the binary
    if curl -L -o "$TEMP_FILE" "$DOWNLOAD_URL" 2>> "$LOGFILE"; then
      log_msg "Download successful, extracting..."
      
      # Decompress the xz file with multiple fallbacks
      DECOMPRESS_SUCCESS=0
      
      if which unxz > /dev/null 2>&1; then
        log_msg "Using unxz for decompression..."
        if unxz -f "$TEMP_FILE" >> "$LOGFILE" 2>&1; then
          EXTRACTED_FILE="$DATADIR/frida-server"
          DECOMPRESS_SUCCESS=1
        else
          log_msg "ERROR: unxz decompression failed"
        fi
      elif which xz > /dev/null 2>&1; then
        log_msg "Using xz for decompression..."
        if xz -d -f "$TEMP_FILE" >> "$LOGFILE" 2>&1; then
          EXTRACTED_FILE="$DATADIR/frida-server"
          DECOMPRESS_SUCCESS=1
        else
          log_msg "ERROR: xz decompression failed"
        fi
      elif which 7z > /dev/null 2>&1; then
        log_msg "Using 7z for decompression..."
        if 7z x "$TEMP_FILE" -o"$DATADIR" >> "$LOGFILE" 2>&1; then
          EXTRACTED_FILE="$DATADIR/frida-server"
          DECOMPRESS_SUCCESS=1
        else
          log_msg "ERROR: 7z decompression failed"
        fi
      elif which busybox > /dev/null 2>&1; then
        log_msg "Using busybox xzcat for decompression..."
        if busybox xzcat "$TEMP_FILE" > "$DATADIR/frida-server" 2>> "$LOGFILE"; then
          EXTRACTED_FILE="$DATADIR/frida-server"
          DECOMPRESS_SUCCESS=1
        else
          log_msg "ERROR: busybox xzcat decompression failed"
        fi
      else
      ...
      if [ "$DECOMPRESS_SUCCESS" -eq 1 ] && [ -f "$EXTRACTED_FILE" ]; then
      ...
      log_msg "ERROR: Failed to download Frida (attempt $((retry_count + 1))/$MAX_RETRIES)"
      rm -f "$TEMP_FILE"
    fi
    ...
    if [ $retry_count -lt $MAX_RETRIES ]; then
      log_msg "Waiting ${RETRY_DELAY}s before retry..."
      sleep $RETRY_DELAY
    fi
  done
  
  log_msg "ERROR: Failed to download Frida after $MAX_RETRIES attempts"
  return 1
}
